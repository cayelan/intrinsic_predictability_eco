---
title: The intrinsic predictability of ecological time series and its potential to
  guide forecasting
...
#devtools::install_github("thomasp85/patchwork")# actually installing from CRAN instead
...
pdf("../Figs/EDM_fitting.pdf")

#need to skip dataset 56
dd_ts <- dd_ts[-c(56,77, 78,79, 80, 174, 375, 376),]
...
#this was originally what was here: I think bad because of changes in this function 
  #due to updates to rEDM package since when the paper was submitted
  
  #my modified (fixed) version
  simplex_fit <- 
    simplex(ts_select$abundance_sc, norm = 2,
                         lib = c(1, (length(ts_select$abundance_sc)-step_ahead)),
                         stats_only = T, E = 1:10,
              silent=T)
  ...
  temp<-as.vector(unlist(simplex_fit$rho)) #add in an na_exclude statement?
  E_hat_rho<-which(temp==max(temp))
...
#  abline(v=E_hat_rho, col="red", lty=2)
  ...
  temp1<-as.vector(unlist(smap_fit$rho)) #add in an na_exclude statement?
  theta_hat_rho <- smap_fit$theta[which(temp1==max(temp1))]
  ...
  print("smap_pred")
   ...
   plot(ts_select$time, ts_select$abundance_sc, type="l", xlab = "Time", ylab="Abundance") 
         ...
         #                 ts_select$abundance_sc, na.rm=T), 
          ...
          #           ts_select$abundance_sc, na.rm=T)))
     points(smap_pred$model_output[[1]]$Index, 
            smap_pred$model_output[[1]]$Predictions, col=2)
         ...
         mtext(paste0("MainID = ", dd_ts$MainID[i]), side=3, line=-2, cex=1, outer=T)
           ...
           # ylim =range(min(smap_pred$model_output[[1]]$pred, 
            ...
            #            max(smap_pred$model_output[[1]]$pred,
             #               ts_select$abundance_sc, na.rm=T)))
      #lines(smap_pred$model_output[[1]]$Index, smap_pred$model_output[[1]]$Predictions, 
       #     col=2, ylim =range(
...
```{r}
E_type <- ggplot(data=dd_ts, aes(x=stats$E, group=type, fill=type)) + 
  geom_density(position="identity", alpha=.3) + 
  guides(alpha=F) + 
  theme(legend.position = "bottom")
theta_type <- ggplot(data=dd_ts, aes(x=stats$theta, group=type, fill=type)) + 
...
```{r}
for(i in 1:nrow(dd_ts)){
dd_ts$nrmse[i] <- unlist((dd_ts$stats$rmse[i]))/
    (max(dd_ts$data[[i]]$abundance_sc, na.rm=T) - 
      min(dd_ts$data[[i]]$abundance_sc, na.rm=T)) 
}    
...
dd_ts$rmse_sqrt <- sqrt(unlist(dd_ts$stats$rmse))
dd_ts$PE_sqrt <- sqrt(dd_ts$PE)
# simplify data frame before model fitting
dd_ts_simple <- dd_ts %>% 
  dplyr::select(nrmse_sqrt, rmse_sqrt, 
                         PE, 
                         type, 
                         N, 
                         zero_prop, 
                         ties_prop, 
                         number_of_gaps, source) 
 dd_ts_simple1 <- bind_cols(dd_ts_simple,
                         dd_ts$stats$E, 
                         dd_ts$stats$theta)
 colnames(dd_ts_simple1)[10]<- "E"
 colnames(dd_ts_simple1)[11]<- "theta"
...
dd_ts2 <- bind_cols(dd_ts_simple1, dd_ts[,c("nrmse", "label")])
...
lme_fit <- 
  nlme::lme(nrmse_sqrt ~ PE * type + N +  
            ...
            data = dd_ts2)
dat <-ggeffects::ggemmeans(lme_fit, terms = c("PE"), type = c("fe"))
dat$label <- NA
